...
| 4 | Complementary filter → gravity / `jerk`. **Authored Nest backdrop.** Tilt does **not** orbit. Shake hops a debug mass. **Product test.** | [04 IMU](./04-imu-qmi8658.md) [05 fusion](./05-fusion-gravity-camera.md) |
| 5 | One L2 part **mesh**, continuous yaw, raster at `project(pos)` with N·L bands. Room stays put. `view_idx` is order/overlay, not a photo | [05](./05-fusion-gravity-camera.md) |
| 6 | Six springs + one idle rest. Sphere poke (S). Play toy RBs. Dummy Hall L0 combined mesh. Door cut | [06 touch stub](./06-touch-cst816.md) |
| 7 | Battery idle / GRAM-hold (springs settled, RBs sleeping, `fx_live==0`). Studio vs battery backlight. Overlay: awake RB count | [08 power stub](./08-power-battery.md) |
